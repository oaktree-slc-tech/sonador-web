...
class SonadorCredentialProvider(SonadorCredentialBaseProvider):
	...
	def decode_server_token_authdata(self, cleaned_data, tokenvalue_kw='token_value'):
		...
		try:
			...
			logger.error('%s: Unable to decrypt server token from the provided value, bad signature.' % type(self).__name__)
	...
	def decode_session_authdata(self, cleaned_data, tokenvalue_kw='token_value'):
		...
		try:
			...
			logger.error('%s: Unable to retrieve session ID from the token, mismatched signature. Token: "%s"'
				% (type(self).__name__, masked_value(cleaned_data.get(tokenvalue_kw) or '')))
	...
	def basicauth_check_accessid(self, cleaned_data, tokenvalue_kw='token_value'):
		...
		try:
			...
			logger.error('%s: Unable to decode user credentials from authentication string'
				% type(self).__name__)

		except ApiAccess.DoesNotExist:
			logging.error('%s: Unable to retrieve user credentials, invalid access ID'
				% type(self).__name__)

		return cleaned_data

	def clean(self):
		'''	Decode and parse the credential to a user instance
		'''
		...
		# 2. The value includes "api-token"
			...
			or API_ACCESS_APITOKEN_QSPARAM in (cleaned_data.get(tokenvalue_kw) or '').lower():
			...
			try:
				...
				logger.error('%s: Unable to retrieve API token matching request: %s' 
					% (type(self).__name__, cleaned_data.get(tokenvalue_kw)))

		# Throw a validation error if unable to retrieve user for token
		if not getattr(self, 'user'):
			raise CredentialValidationError('Unable to decode and retrieve user for type=%s token=%s' 
				% (cleaned_data.get('token_type'), cleaned_data.get(tokenvalue_kw)))
